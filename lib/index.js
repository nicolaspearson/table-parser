  ...
  valuesDomainsVerticalGroups.forEach(function (group) {
    ...
    for (title in titleInfo) {
      ...
      if (group.begin > info.titleBegin) {
        ...
        if (!nearestLeftTitle || ( distance < minimumLeftDistance )) {
          nearestLeftTitle = title;
          minimumLeftDistance = distance;
        }
      }
...
/**
 * transform a string value into array. It's not just split(), but also to consider some chunk that wrapped with `"`
 */
